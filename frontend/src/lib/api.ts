...
async function directApi(path:string,init:RequestInit):Promise<any|typeof MISS>{
  ...
  if(p==='/dashboard'&&method==='GET'){
    const [settings,focus,tasks,programs,decisions]=await Promise.all([table('project_settings').select('*').eq('workspace_id',ws).maybeSingle(),table('weekly_focus_items').select('*').eq('workspace_id',ws).order('week_start',{ascending:false}).order('created_at').limit(6),table('tasks').select('*').eq('workspace_id',ws).eq('archived',false).neq('status','done').order('blocked',{ascending:false}).order('due_date',{ascending:true,nullsFirst:false}).order('created_at',{ascending:false}).limit(12),table('programs').select('*').eq('workspace_id',ws).eq('archived',false).order('deadline',{ascending:true,nullsFirst:false}).limit(8),table('decisions').select('*').eq('workspace_id',ws).eq('archived',false).order('decision_date',{ascending:false}).order('created_at',{ascending:false}).limit(5)])
